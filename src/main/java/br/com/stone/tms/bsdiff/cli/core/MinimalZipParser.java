...
class MinimalZipParser {
	 ...
	 * ZIP-like archive. This is in little-endian order.
	 */
	public static final int EOCD_SIGNATURE = 0x06054b50;

	/**
	 * Standard 32-bit signature for a "central directory entry" record in a
	 * ZIP-like archive. This is in little-endian order.
	 */
	...
	/**
	 * Skips exactly the specified number of bytes, throwing an exception if
	 * unsuccessful.
	 * 
	 * @param in       the stream to read from
	...
	/**
	 * Reads 2 bytes from the current offset as an unsigned, 32-bit little-endian
	 * value.
	 * 
	 * @param in the stream to read from
	...
	/**
	 * Reads 4 bytes from the current offset as an unsigned, 32-bit little-endian
	 * value.
	 * 
	 * @param in the stream to read from
	...
	/**
	 * Read exactly the specified amount of data into the specified buffer, throwing
	 ...
	 * @param length the number of bytes to place into the buffer from the input
	 *               stream
	 * @throws IOException if unable to read
	 */
	 ...
	 * 
	 * @param in the input stream to read from, assumed to start at the first byte
	 *           of the entry
	...
	public static MinimalZipEntry parseCentralDirectoryEntry(InputStream in) throws IOException {
		...
		// *** 2 bytes encode the general-purpose flags, read for language encoding.
		// [READ THIS]
		...
		// *** 2 bytes encode the length of the extras, needed to skip the bytes later
		// [READ THIS]
		// *** 2 bytes encode the length of the comment, needed to skip the bytes later
		// [READ THIS]
		...
		// *** 4 bytes encode the offset of the local section entry, where the data is
		// [READ THIS]
		...
		// General purpose flag bit 11 is an important hint for the character set used
		// for file names.
		boolean generalPurposeFlagBit11 = (generalPurposeFlags & (0x1 << 10)) != 0;
		return new MinimalZipEntry(compressionMethod, crc32OfUncompressedData, compressedSize, uncompressedSize,
				fileNameBuffer, generalPurposeFlagBit11, fileOffsetOfLocalEntry);
	}

	/**
	 * Parses one local file entry and returns the offset from the first byte at
	 * which the compressed data begins
	 * 
	 * @param in the input stream to read from, assumed to start at the first byte
	 *           of the entry
	 * @return as described
	...
	public static long parseLocalEntryAndGetCompressedDataOffset(InputStream in) throws IOException {
		...
		// 2 bytes encode the compression method, ignore (redundant with central
		// directory)
		// 2 bytes encode the MSDOS last modified file time, ignore
		// 2 bytes encode the MSDOS last modified file date, ignore
		// 4 bytes encode the CRC32 of the uncompressed data, ignore (redundant with
		// central directory)
		// 4 bytes encode the compressed size, ignore (redundant with central directory)
		// 4 bytes encode the uncompressed size, ignore (redundant with central
		// directory)
		// *** 2 bytes encode the length of the file name, needed to skip the bytes
		// later [READ THIS]
		// *** 2 bytes encode the length of the extras, needed to skip the bytes later
		// [READ THIS]
		...
		// The file name is already known and will match the central directory, so no
		// need to read it.
		// The extra field length can be different here versus in the central directory
		// and is used for
		// things like zipaligning APKs. This single value is the critical part as it
		// dictates where the
	 ...
	 * of a file looking for the signature of the record.
	 * 
	 * @param in                 the file to read from
	 * @param searchBufferLength the length of the search buffer, starting from the
	 *                           end of the file
	 * @return the offset in the file at which the first byte of the EOCD signature
	 *         is located, or -1 if the signature is not found in the search buffer
	...
	/**
	 * Find the end-of-central-directory record by scanning backwards looking for
	 * the signature of the record.
	 * 
	 * @param buffer the buffer in which to search
	 * @return the offset in the buffer at which the first byte of the EOCD
	 *         signature is located, or -1 if the complete signature is not found
	 */
	...
	/**
	 * Parse the end-of-central-directory record and return the critical information
	 * from it.
	 * 
	 * @param in the input stream to read from, assumed to start at the first byte
	 *           of the entry
	 * @return the metadata
	 * @throws IOException  if unable to read
	 * @throws ZipException if the metadata indicates this is a zip64 archive, which
	 *                      is not supported
	 */
	public static MinimalCentralDirectoryMetadata parseEocd(InputStream in) throws IOException, ZipException {
		...
		// Everything else from here to the EOF is the zip file comment, or junk.
		// Ignore.
		skipOrDie(in, 2 + 2 + 2);
		...
		return new MinimalCentralDirectoryMetadata(numEntriesInCentralDirectory, offsetOfCentralDirectory,
