...
import br.com.stone.tms.bsdiff.cli.core.similarity.Crc32SimilarityFinder;
import br.com.stone.tms.bsdiff.cli.core.similarity.SimilarityFinder;
...
class PreDiffPlanner {	
	...
	PreDiffPlanner(File oldFile, Map<ByteArrayHolder, MinimalZipEntry> oldArchiveZipEntriesByPath, File newFile,
	...
	/**
	 * The divined parameters for compression of the entries in the new archive,
	 * with paths as keys.
	 */
	private final Map<ByteArrayHolder, JreDeflateParameters> newArchiveJreDeflateParametersByPath;

	/**
	 * Optional {@link RecommendationModifier}s that will be applied after the
	 * default recommendations have been made but before the {@link PreDiffPlan} is
	 * constructed.
	 */
	private final List<RecommendationModifier> recommendationModifiers;

	/**
	 * Generates and returns the plan for archive transformations to be made prior
	 * to differencing. The resulting {@link PreDiffPlan} has the old and new file
	 * uncompression plans set. The delta-friendly new file recompression plan is
	 * <em>not</em> set at this time.
	 * 
	 * @return the plan
	...
	PreDiffPlan generatePreDiffPlan() throws IOException {
		...
		for (QualifiedRecommendation recommendation : recommendations) {
			...
			if (recommendation.getRecommendation().uncompressNewEntry) {
				...
				JreDeflateParameters newJreDeflateParameters = newArchiveJreDeflateParametersByPath
						.get(new ByteArrayHolder(recommendation.getNewEntry().getFileNameBytes()));
				TypedRange<JreDeflateParameters> range = new TypedRange<JreDeflateParameters>(offset, length,
						newJreDeflateParameters);
		...
		return new PreDiffPlan(Collections.unmodifiableList(recommendations),
				Collections.unmodifiableList(oldFilePlanList), Collections.unmodifiableList(newFilePlanList));
	}

	/**
	 * Analyzes the input files and returns the default recommendations for each
	 * entry in the new archive.
	 *
	...
	private List<QualifiedRecommendation> getDefaultRecommendations() throws IOException {
		...
		// This will be used to find files that have been renamed, but not modified.
		// This is relatively
		// cheap to construct as it just requires indexing all entries by the
		// uncompressed CRC32, and
		// the CRC32 is already available in the ZIP headers.
		SimilarityFinder trivialRenameFinder = new Crc32SimilarityFinder(oldFile, oldArchiveZipEntriesByPath.values());

		// Iterate over every pair of entries and get a recommendation for what to do.
		for (Map.Entry<ByteArrayHolder, MinimalZipEntry> newEntry : newArchiveZipEntriesByPath.entrySet()) {
			ByteArrayHolder newEntryPath = newEntry.getKey();
			MinimalZipEntry oldZipEntry = oldArchiveZipEntriesByPath.get(newEntryPath);
			if (oldZipEntry == null) {
				// The path is only present in the new archive, not in the old archive. Try to
				// find a
				// similar file in the old archive that can serve as a diff base for the new
				// file.
				List<MinimalZipEntry> identicalEntriesInOldArchive = trivialRenameFinder.findSimilarFiles(newFile,
						newEntry.getValue());
				if (!identicalEntriesInOldArchive.isEmpty()) {
					// An identical file exists in the old archive at a different path. Use it for
					// the
					...
					// NB, in principle it would be optimal to select the file that required the
					// least work
					// to apply the patch - in practice, it is unlikely that an archive will contain
					// multiple
					// copies of the same file that are compressed differently, so don't bother with
					// that
					// degenerate case.
			...
			// If the attempt to find a suitable diff base for the new entry has failed,
			// oldZipEntry is
			// null (nothing to do in that case). Otherwise, there is an old entry that is
			// relevant, so
	...
	/**
	 * Determines the right {@link QualifiedRecommendation} for handling the
	 * (oldEntry, newEntry) tuple.
	 * 
	...
	private QualifiedRecommendation getRecommendation(MinimalZipEntry oldEntry, MinimalZipEntry newEntry)
		...
		if (unsuitableDeflate(newEntry)) {
			return new QualifiedRecommendation(oldEntry, newEntry, Recommendation.UNCOMPRESS_NEITHER,
		...
		// 2. The new entry is either uncompressed, or is reproducibly compressed with
		// deflate.

		if (uncompressedChangedToCompressed(oldEntry, newEntry)) {
			return new QualifiedRecommendation(oldEntry, newEntry, Recommendation.UNCOMPRESS_NEW,
					RecommendationReason.UNCOMPRESSED_CHANGED_TO_COMPRESSED);
		}

		if (compressedChangedToUncompressed(oldEntry, newEntry)) {
			return new QualifiedRecommendation(oldEntry, newEntry, Recommendation.UNCOMPRESS_OLD,
		...
		if (compressedBytesChanged(oldEntry, newEntry)) {
			return new QualifiedRecommendation(oldEntry, newEntry, Recommendation.UNCOMPRESS_BOTH,
	...
	/**
	 * Returns true if the entries are unsuitable for doing an uncompressed diff.
	 * This method returns true if either of the entries is compressed in an
	 * unsupported way (a non-deflate compression algorithm).
	 *
	 * @param oldEntry the entry in the old archive
	 * @param newEntry the entry in the new archive
	...
	private boolean unsuitable(MinimalZipEntry oldEntry, MinimalZipEntry newEntry) {
		if (oldEntry.getCompressionMethod() != 0 && !oldEntry.isDeflateCompressed()) {
			// The old entry is compressed in a way that is not supported. It cannot be
			// uncompressed, so
		...
		if (newEntry.getCompressionMethod() != 0 && !newEntry.isDeflateCompressed()) {
			// The new entry is compressed in a way that is not supported. Same result as
			// above.
			return true;
		}
		return false;
	...
	/**
	 * Returns true if the entries are unsuitable for doing an uncompressed diff as
	 * a result of the new entry being compressed via deflate, with undivinable
	 * parameters. This could be the result of an unsupported version of zlib being
	 * used.
	 *
	 * @param newEntry the entry in the new archive
	 * @return true if unsuitable
	...
	private boolean unsuitableDeflate(MinimalZipEntry newEntry) {
		JreDeflateParameters newJreDeflateParameters = newArchiveJreDeflateParametersByPath
				.get(new ByteArrayHolder(newEntry.getFileNameBytes()));
		if (newEntry.isDeflateCompressed() && newJreDeflateParameters == null) {
			// The new entry is compressed via deflate, but the parameters were undivinable.
			// Therefore the
	...
	/**
	 * Returns true if the entries are already optimal for doing an uncompressed
	 * diff. This method returns true if both of the entries are already
	 * uncompressed, i.e. are already in the best form for diffing.
	 * 
	 * @param oldEntry the entry in the old archive
	 * @param newEntry the entry in the new archive
	 * @return as described
	...
	/**
	 * Returns true if the entry is uncompressed in the old archive and compressed
	 * in the new archive. This method does not check whether or not the compression
	 * is reproducible. It is assumed that any compressed entries encountered are
	 * reproducibly compressed.
	 * 
	 * @param oldEntry the entry in the old archive
	 * @param newEntry the entry in the new archive
	 * @return as described
	...
	/**
	 * Returns true if the entry is compressed in the old archive and uncompressed
	 * in the new archive. This method does not check whether or not the compression
	 * is reproducible because that information is irrelevant to this decision (it
	 * does not matter whether the compression in the old archive is reproducible or
	 * not, because that data does not need to be recompressed at patch apply time).
	 * 
	 * @param oldEntry the entry in the old archive
	 * @param newEntry the entry in the new archive
	 * @return as described
	...
	/**
	 * Checks if the compressed bytes in the specified entries have changed. No
	 * attempt is made to inflate, this method just examines the raw bytes that
	 * represent the content in the specified entries and returns true if they are
	 * different.
	 * 
	 * @param oldEntry the entry in the old archive
	 * @param newEntry the entry in the new archive
	...
	private boolean compressedBytesChanged(MinimalZipEntry oldEntry, MinimalZipEntry newEntry) throws IOException {
		...
		int numRead = 0;
		try (RandomAccessFileInputStream newRafis = new RandomAccessFileInputStream(newFile,
				newEntry.getFileOffsetOfCompressedData(), newEntry.getCompressedSize());
				MatchingOutputStream matcher = new MatchingOutputStream(new RandomAccessFileInputStream(oldFile,
						oldEntry.getFileOffsetOfCompressedData(), oldEntry.getCompressedSize()), 4096)) {
